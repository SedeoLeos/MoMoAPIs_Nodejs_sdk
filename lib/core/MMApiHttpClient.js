...
class MMApiHttpClient extends HttpClient {
  ...
  ObtainAnAccessTokenRequest = async () => {
    ...
    return super.execute(new ObtainAnAccessTokenRequest(this.environment))
      .then((response) => {
...
function authInjector(request) {
  ...
  if (request.constructor.name === 'ObtainAnAccessTokenRequest' || request.constructor.name === 'CreateAccessToken') {
    return;
  }
